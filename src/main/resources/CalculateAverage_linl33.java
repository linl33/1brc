...
public class CalculateAverage_linl33 {
    private static final String FILE = "./measurements_1g.txt";
    ...
    private static final Path FILE_PATH = Paths.get(FILE);
    ...
    // private static final MethodHandle OF_ADDRESS_HANDLE;
    ...
    // private static MemorySegment ofAddress(final long address) {
    // return MemorySegment.ofAddress(address).reinterpret(128L);
    // }

    // private static VarHandle adaptSegmentVarHandle(VarHandle handle) {
    // handle = MethodHandles.insertCoordinates(handle, 1, 0L);
    // handle = MethodHandles.filterCoordinates(handle, 0, OF_ADDRESS_HANDLE);
    // return handle;
    // }
...
//        try {
//            OF_ADDRESS_HANDLE = MethodHandles
//                    .lookup()
//                    .findStatic(
//                            CalculateAverage_linl33.class,
//                            "ofAddress",
...
//        }
//
//        // create var handles to access memory without bounds check
//        // from https://mail.openjdk.org/pipermail/panama-dev/2023-July/019487.html
//        INT_HANDLE = adaptSegmentVarHandle(ValueLayout.JAVA_INT_UNALIGNED.varHandle());
//        LONG_HANDLE = adaptSegmentVarHandle(ValueLayout.JAVA_LONG_UNALIGNED.varHandle());
//        BYTE_HANDLE = adaptSegmentVarHandle(ValueLayout.JAVA_BYTE.varHandle());
    }

    public static void main() throws InterruptedException, IOException {
        try (final var channel = FileChannel.open(FILE_PATH)) {
    ...
    /**
     * Open addressing, linear probing hash map back by off-heap memory
     */
    private static class SparseMap {
        ...
        private static boolean mismatch(final long leftAddr, final long rightAddr, final int length) {
            ...
            // to platforms without TZCNT, the check can be replaced with
